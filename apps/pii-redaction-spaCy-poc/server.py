"""
PII Redaction WebSocket Proxy Server
=====================================
...
Architecture:
  Client <-> [This Server] <-> OpenAI Realtime API
                  |
          Injection Guard
          PII Redactor (spaCy + Regex)

Run:
  # Full proxy mode (requires OPENAI_API_KEY):
  uvicorn server:app --host 0.0.0.0 --port 8000

  # Dry-run mode (no OpenAI needed — great for demos):
  DRY_RUN=true uvicorn server:app --host 0.0.0.0 --port 8000

Scaling path:
  - POC: Run locally, spaCy in-process
  - Production: Deploy to ECS Fargate, NER on SageMaker endpoint
"""
...
async def _process_client_messages(
    ...
    If openai_ws is None (dry-run mode), we send results back to the
    client directly instead of forwarding to OpenAI. This makes the
    security layers fully testable without an API key.
    """
    try:
        while True:
